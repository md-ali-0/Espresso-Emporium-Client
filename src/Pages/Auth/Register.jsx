...
const Register = () => {
    const { user, createSignUp, createGoogleLogin, setLoading } = useContext(AuthContext);
                ...
                <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form className="space-y-4 md:space-y-6" onSubmit={handleRegistration}>
                        ...
                        <div>
                            ...
                            <input type="password" name="password" id="password" className="bg-gray-50 border focus:outline-none border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-0 focus:border-primary font-raleway w-full p-2.5" placeholder="Enter Password" required />
                        </div>
                        {err ? <p className="text-red-600 text-center">{err}</p> : ''}
                        <button type="submit" className="w-full font-raleway text-white bg-primary hover:bg-primary focus:outline-none focus:ring-0 font-medium rounded-lg text-lg px-5 py-2 text-center">Sign Up</button>

                    </form>
                    <div>
                        <p className="text-center text-gray-700 text-sm pb-3">Or</p>
                        <div className="space-y-3">
                        <button onClick={handleGoogleLogin} className="w-full border border-gray-200 focus:outline-none focus:ring-0 font-medium rounded-lg text-sm px-5 py-2 text-center flex items-center justify-center font-raleway gap-2"> <span className="text-3xl "><FcGoogle size={20}></FcGoogle></span> Sign Up With Google</button>
                        </div>
                    </div>
